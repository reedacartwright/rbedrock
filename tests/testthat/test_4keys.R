...
test_that("chrkeys_to_rawkeys supports actor digest keys keys", {
    expect_equal(chrkeys_to_rawkeys(c("acdig:-1:1:0","acdig:-1:1:1",
        "acdig:-1:1:2","acdig:-1:1:0")),
        list(as.raw(c(0x64,0x69,0x67,0x70,0xff,0xff,0xff,0xff,1,0,0,0)),
             as.raw(c(0x64,0x69,0x67,0x70,0xff,0xff,0xff,0xff,1,0,0,0,1,0,0,0)),
             as.raw(c(0x64,0x69,0x67,0x70,0xff,0xff,0xff,0xff,1,0,0,0,2,0,0,0)),
             as.raw(c(0x64,0x69,0x67,0x70,0xff,0xff,0xff,0xff,1,0,0,0))
        ))
})

test_that("chrkeys_to_rawkeys treats invalid actor digest keys keys as strings", {
...
test_that("chrkeys_to_rawkeys supports actor keys", {
    ...
    expect_equal(chrkeys_to_rawkeys(c("actor:0123456789abcdef","actor:FEDCBA9876543210")),
        list(as.raw(c(raw_prefix, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef)),
             as.raw(c(raw_prefix, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10))
        ))
})
...
test_that("rawkeys_to_chrkeys supports NA and NULL", {
    expect_equal(rawkeys_to_chrkeys(list(NULL,charToRaw("test"))), c(NA_character_,"plain:test"))
...
test_that("rawkeys_to_charkeys supports percent encoding", {
    expect_equal(rawkeys_to_chrkeys(charToRaw("@0:0:0:0")), "plain:@0:0:0:0")
    expect_equal(rawkeys_to_chrkeys(charToRaw("Hello%World")), "plain:Hello%25World")
    expect_equal(rawkeys_to_chrkeys(charToRaw("Hello\x4AWorld")), "plain:HelloJWorld")
    expect_equal(rawkeys_to_chrkeys(charToRaw("Hello\x0FWorld")), "plain:Hello%0FWorld")
    expect_equal(rawkeys_to_chrkeys(charToRaw("Hello\x80World")), "plain:Hello%80World")
})
...
test_that("rawkeys_to_chrkeys supports chunk keys", {
    expect_equal(rawkeys_to_chrkeys(
        ...
        c("chunk:-1:1:0:50","chunk:-1:1:1:47:3","chunk:-1:1:2:50","chunk:-1:1:0:47:3",
            "chunk:-1:1:0:47:-1","chunk:-1:1:0:47:-4"))
})
...
test_that("parse_chunk_keys parses correctly",{
    expect_equal(parse_chunk_keys("chunk:0:0:0:47:1"),
        tibble::tibble(key="chunk:0:0:0:47:1",x=0L,z=0L,dimension=0L,tag="SubChunkBlocks",subtag=1L))
})
