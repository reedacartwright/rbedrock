...
test_that("put_biomes_value accepts strings", {
    set.seed(102031)
    put_biomes_value(db, 32, 3, 0, "plains")
    dat <- get_biomes_value(db, 32, 3, 0)
    expect_equal(dat, matrix("plains", 16L, 16L))

    new_dat <- sample(c("plains","desert","ocean"),256,replace=TRUE)
    put_biomes_value(db, 32, 3, 0, new_dat)
    dat <- get_biomes_value(db, 32, 3, 0)
    expect_equal(dat, matrix(new_dat, 16L, 16L))

    expect_error(put_biomes_value(db, 32, 3, 0, "fake_biome"))
})

test_that("put_biomes_value accepts integers", {
    set.seed(102032)
    put_biomes_value(db, 32, 3, 0, 1L)
    dat <- get_biomes_value(db, 32, 3, 0, return_names=FALSE)
