...
test_that("get_acdig_value() returns a vector of actor keys.", {
  ...
  expect_equal(
    dat,
    c(
      "actor:000000010000000A",
      "actor:000000010000000B",
...
test_that("get_acdig_data() returns a list of vectors of actor keys.", {
  dat <- get_acdig_data(c(0, -7, 1, 1), c(0, -2, 0, 0), c(0, 0, 0, 1), db = db)
  expected <- list(
    "acdig:0:0:0" = c(
      "actor:000000010000000A",
      "actor:000000010000000B",
...
test_that("put_acdig_value() writes actor digest data to db.", {
  ...
  expect_error(write_acdig_value(c(
    "actor:000000020000000A",
...
test_that("put_acdig_data() writes actor digest data to db.", {
  ...
  expected <- list(
    "acdig:0:1:1" = as_raw(0, 0, 0, 2, 0, 0, 0, 10, 0, 0, 0, 2, 0, 0, 0, 11),
    "acdig:0:2:1" = as_raw(0, 0, 0, 2, 0, 0, 0, 12, 0, 0, 0, 2, 0, 0, 0, 13)
  )
  expect_equal(dat, expected)
...
test_that("get_actors_data() returns a list of lists of NBT data.", {
  ...
  expect_named(dat[[2]], c("actor:000000010000000F", "actor:0000000100000010"))
...
test_that("put_actors_value() writes actors data.", {
  ...
  actors_data <- list(
    "acdig:200:0:0" = list(
      "actor:000000000000005A" = actor_a,
