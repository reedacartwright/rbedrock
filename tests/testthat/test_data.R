...
test_that("get_value requires a scalar character argument", {
    ...
    expect_error(get_value(c("hello", "world"), db = db),
        class = "vctrs_error_assert_size")
})

test_that("get_data returns a list of raw values", {
    expect_equal(get_data(c("chunk:31:6:0:44", "plain:Overworld",
        "plain:NotFound"), db = db),
...
test_that("has_values returns a logical vector", {
    expect_equal(has_values(c("chunk:31:6:0:44", "plain:Overworld",
        "plain:NotFound"), db = db),
        c("chunk:31:6:0:44" = TRUE, "plain:Overworld" = TRUE,
...
test_that("put_value throws errors are incorrect arguments", {
    ...
    expect_error(put_value(raw(1L), as.raw(1L), db = db),
                 class = "vctrs_error_assert_ptype")
    expect_error(put_value(raw(1L), c("plain:hello", "plain:world"), db = db),
                 class = "vctrs_error_assert_size")
...
test_that("put_data stores data into the db", {
    put_data(list("plain:Test%01" = as.raw(0x1),
                  "plain:Test%02" = as.raw(0x2)),
             db = db)
    put_result <- get_data(c("plain:Test%01", "plain:Test%02"), db = db)
    expect_equal(put_result, list("plain:Test%01" = as.raw(0x1),
                                  "plain:Test%02" = as.raw(0x2)))

    put_data(list(as.raw(0x10), as.raw(0x20)), c("plain:Test%01",
                                                 "plain:Test%02"),
             db = db)
    put_result <- get_data(c("plain:Test%01", "plain:Test%02"), db = db)
    expect_equal(put_result, list("plain:Test%01" = as.raw(0x10),
                                  "plain:Test%02" = as.raw(0x20)))
})

test_that("delete_data removes data from the db", {
    put_data(list("plain:Test%01" = as.raw(0x1),
                  "plain:Test%02" = as.raw(0x2)), db = db)
    delete_data(c("plain:Test%01", "plain:Test%02"), db = db)
    delete_result <- get_data(c("plain:Test%01", "plain:Test%02"), db = db)
    expect_equal(delete_result, list("plain:Test%01" = NULL,
                                     "plain:Test%02" = NULL))
})
...
test_that("write_data writes data into a db", {
    ...
    expect_equal(has_values(names(dat), db = db),
                 set_names(c(FALSE, TRUE, FALSE), names(dat)))
