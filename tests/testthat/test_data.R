...
test_that("get_values returns a list of raw values", {
    expect_equal(get_values(db, c("chunk:31:6:0:44","plain:Overworld","plain:NotFound")),
...
test_that("has_values returns a logical vector", {
    expect_equal(has_values(db, c("chunk:31:6:0:44","plain:Overworld","plain:NotFound")),
...
test_that("put_values writes data into the db", {
    put_values(db, c("plain:Test%01", "plain:Test%02"), list(as.raw(0x1), as.raw(0x2)))
...
test_that("put_data writes data into the db", {
    put_data(db, list("plain:Test%01" = as.raw(0x1), "plain:Test%02" = as.raw(0x2)))
    put_result <- get_values(db, c("plain:Test%01", "plain:Test%02"))
    expect_equal(put_result, list("plain:Test%01" = as.raw(0x1), "plain:Test%02" = as.raw(0x2)))
})
...
test_that("delete_values removes data from the db", {
    ...
    put_data(db, list("plain:Test%01" = as.raw(0x1), "plain:Test%02" = as.raw(0x2)))
    delete_ret <- delete_values(db, c("plain:Test%01", "plain:Test%03","plain:Test%02"), report=TRUE)
