...
res <- jsonlite::fromJSON(
  "https://api.github.com/repos/gentlegiantJGC/PyMCTranslate/releases/latest"
) # nolint
...
vanilla_blockstates <- file.path(
  bedrock_latest,
  "block/blockstate/specification/minecraft/vanilla"
) # nolint
...
vanilla_block_states_df <- do.call(
  rbind,
  c(blockstates, make.row.names = FALSE)
)
