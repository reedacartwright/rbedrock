...
put_blocks_data <- function(values, x, z, dimension, db = default_db()) {
  keys <- process_chunk_key_args(x, z, dimension, 47L, values = values)
  b <- check_chunk_key_args(keys, 47L)
  mapply(
    values[b],
    keys[b],
    FUN = put_blocks_value_impl,
    MoreArgs = list(db = db),
    SIMPLIFY = FALSE
  )
  invisible(b)
